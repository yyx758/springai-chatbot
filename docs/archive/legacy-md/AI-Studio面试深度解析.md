...
### Q: 怎么保证不同用户的文件不互相看到？（安全隔离）

所有 file-service API 都通过 `X-Auth-UserId` 请求头传递当前用户 ID。`FileService.canAccess()` 方法统一校验文件所属权：
```java
// 上传时绑定用户
...
// 下载/删除前校验所属权
if (!fileService.canAccess(record, requestUserId)) {
    return 403;  // 拒绝访问
}
```
前端通过 `fileServiceFetch()` 统一注入 `X-Auth-UserId` header。`<img>` 标签无法设 header，改用 query param `?userId=` 传参。跨用户文件不可见、不可下载、不可删除。
...
| 文件用户隔离 | FileService.canAccess() | 全方法 |
