...
public class EmailService {
    ...
    public void sendResetCode(String toEmail) {
        String intervalKey = "reset_send_interval:" + toEmail;
        Boolean canSend = redisTemplate.opsForValue().setIfAbsent(intervalKey, "1",
                SEND_INTERVAL_SECONDS, TimeUnit.SECONDS);
        if (!Boolean.TRUE.equals(canSend)) {
            throw new IllegalStateException("发送过于频繁，请60秒后再试");
        ...
        String code = String.format("%06d", new Random().nextInt(999999));
        String key = RESET_CODE_KEY_PREFIX + toEmail;
        redisTemplate.opsForValue().set(key, code, CODE_TTL_MINUTES, TimeUnit.MINUTES);
        ...
        try {
            ...
            helper.setFrom(from);
            helper.setTo(toEmail);
            helper.setSubject("AI Studio - 重置密码");
            helper.setText(buildResetEmailContent(code), true);
            mailSender.send(message);
            log.info("重置密码验证码已发送至 {}，有效期 {} 分钟", toEmail, CODE_TTL_MINUTES);
        } catch (MessagingException e) {
            redisTemplate.delete(key);
    ...
    public boolean verifyResetCode(String email, String code) {
        String key = RESET_CODE_KEY_PREFIX + email;
        Object stored = redisTemplate.opsForValue().get(key);
        if (stored == null) return false;
        if (!code.equals(String.valueOf(stored))) return false;
        redisTemplate.delete(key);
        return true;
    }
    ...
    private String buildResetEmailContent(String code) {
        return """
                <div style="max-width:480px;margin:0 auto;padding:32px 24px;
                    ...
                    <p style="color:#64748b;font-size:14px;">您正在重置密码</p>
                    ...
                    <p style="color:#94a3b8;font-size:12px;">验证码 %d 分钟内有效，如非本人操作请忽略</p>
                </div>
